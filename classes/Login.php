...
class Login extends DBConnection {
	...
	public function login(){
		...
		if($qry->num_rows > 0){
			$res = $qry->fetch_array();
                         $status = "Active now";
                         $this->conn->query("UPDATE users SET status1 = '{$status}' WHERE unique_id={$res['unique_id']}");
			if($res['status'] != 1){
	...
	public function logout(){
            $logout_id = $this->conn->real_escape_string($_GET['logout_id']);
             //$login_id = $_settings->userdata('unique_id');
              $status = "Offline now";
            $this->conn->query("UPDATE users SET status1 = '{$status}' WHERE unique_id='{$logout_id}'");
